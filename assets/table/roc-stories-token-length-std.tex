\begin{table}[!htbp]
    \centering
    \caption{Table reporting the average lengths and standard deviations across the tested models for each of the three experimental settings. On the top are reported the same statistics computed on the reference correct ending. On the left are reported the models tested and on the right are the values for each of the three experiments, 0-shot, 3-shot and 3-shot specifying the answer length. Notice that most models are quite loquacious compared the humans. The metrics are computed using the whitespace tokenizer.}
