\begin{table}[!htbp]
    \centering
    \caption{Table reporting the Wasserstein divergence metric computed with the token distribution of each of the models and experiments that were tested. The divergence is computed with respect to the human crowdsourced reference token distribution. On the left are reported the experimental settings, with/without guidelines and with/without colour information. On the right the metrics computed for each of the 4 number of examples (shots) given in input to the model. From these results we can see that this metric is unable to highlight significant differences, except for Falcon models, for which the distributions are completely different from the human. Tokens distributions are computed using Spacy NLP tokenizer.}
