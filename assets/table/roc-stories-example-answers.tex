\begin{table}[!htbp]
\centering
\caption{Table containing two examples of different models on different contextes. Reported in the second column are the input contextes and in the rightmost column the correct reference endings. In third column are reported the models' unprocessed inputs and their processed endings are in the fourth. In order to evaluate the models, their raw outputs are processed by removing special invalid characters and only the first sentence, marked by the dot (\emph{.}) is considered for evaluation purposes.}
